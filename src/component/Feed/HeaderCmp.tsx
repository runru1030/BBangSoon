    ...
    return (<>
        <Header className="row-container">
            ...
            <Content className="row-container" >
                <FeedWrapper className="col-container" id='feed' onClick={onClickHandler} >
                    <span id="bold">{reviewArr.length}</span>
                    <span>일지</span>
                </FeedWrapper>
                <VisitWrapper className="row-container" id='visit' onClick={onClickHandler} >
                    <span><FontAwesomeIcon icon={faBreadSlice} color="#e2c26e" /></span>
                    <span id="text">{visitCnt}</span>
                </VisitWrapper>
            </Content>
        </Header>
        {editNick && <EditForm setEditNick={setEditNick} onClickEditNick={onClickEditNick} />}
    </>
    )
}
